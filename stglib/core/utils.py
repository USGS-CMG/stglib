...
def trim_bins(*args, **kwargs):
    ...
    pad = extra depth to add to extend or reduce the trim, for instance to account for side lobes
    
    Note that returned data set will have NaNs as the fill, for EPIC and CF compliance,
    make sure the encoding _FillValue is not NaN before writing to_netcdf
    """
    ds = args[0]
