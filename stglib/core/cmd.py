...
def aqdhdr2cdf_parser():
    description = (
        "Convert Aquadopp text files to raw .cdf format. Run this "
...
def aqdcdf2nc_parser():
    description = (
        "Convert raw Aquadopp .cdf format to processed .nc files, "
...
def wvswad2cdf_parser():
    description = (
        "Convert Aquadopp .wad wave files to raw .cdf format. "
...
def rskcdf2nc_parser():
    description = (
        "Convert raw RBR d|wave .cdf format to processed .nc files,"
        " optionally compensating for atmospheric pressure"
    )
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("cdfname", help="raw .CDF filename")
...
def hwlbcsv2cdf_parser():
    description = (
        "Convert HOBO pressure sensor .csv file to raw .cdf format."
...
def iqmat2cdf_parser():
    description = (
        "Convert SonTek IQ .mat file to raw .cdf format. Run this "
...
def exocsv2cdf_parser():
    description = (
        "Convert EXO .csv file to raw .cdf format. Run this script "
...
def lisstcsv2cdf_parser():
    description = (
        "Convert LISST .csv file to raw .cdf format. Run this script "
        "from the directory containing LIST file"
    )
    parser = argparse.ArgumentParser(description=description)
    gattsarg(parser)
    yamlarg(parser)
...
def exoturnaround_parser():
    description = (
        "Create YSI EXO turnaround plots. Run this script from "
