...
def load_wad(ds):
    ...
    if (
        "num_wave_bursts" in ds.attrs
    ):  # we can override the number of samples if need be
        print(
            "Overriding number of samples using attr num_wave_bursts of {}".format(
                ds.attrs["num_wave_bursts"]
            )
        )
        ...
        print(
            "Number of rows read is not a multiple of %d. Truncating data "
            "to last full burst" % ds.attrs["WaveNumberOfSamples"]
        )
        ds = ds.sel(
            time=ds.time[0 : int(np.floor(r / ds.attrs["WaveNumberOfSamples"]))]
        )
