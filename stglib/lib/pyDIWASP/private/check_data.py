...
def check_data(DDS, type_):
    ...
    if type_ == 1:
        if type(DDS) != dict:
            print("DIWASP data_check: Instrument data type is not a " "dictionary")
        nc = 1
        if "layout" in DDS:
        ...
        if (
            "datatypes" not in DDS
            or type(DDS["datatypes"])
            not in (list, np.ndarray)  # or np.squeeze(DDS['datatypes']).ndim != 1
            or max(np.shape(DDS["datatypes"])) != nc
        ):
            error = "datatypes"
        else:
            DDS["datatypes"] = np.reshape(DDS["datatypes"], np.size(DDS["datatypes"]))
    ...
    if type_ == 2:
        if type(DDS) != dict:
            print("DIWASP data_check: Special matrix data type is not a " "structure")
    ...
    if type_ == 3:
        if type(DDS) != dict:
            print(
                "DIWASP data_check: Estimation parameter data type is not a "
                "structure"
            )
