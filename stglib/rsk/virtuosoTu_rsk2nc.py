...
def virtuosoTu_rsk_to_cdf(metadata):
    """
    Main function to load data from RSK file and save to raw .CDF
    """

    ds = virtuosoTu_rsk_to_xr(metadata)
...
def virtuosoTu_rsk_to_xr(metadata):
    """
    Load data from RSK file and generate an xarray Dataset
    the imput metadata comes from a dictionary as follows:
        gattfile = "..\\glob_att1110.txt" # this is a global attribute file
        # which contains experiment information such as location, dates, etc.
        metadata = stglib.read_globalatts(gattfile)
        # the basefile is the name of the .rsk data file
        metadata['basefile'] = '054276_20180618_1614'
        metadata['initial_instrument_height'] = 0.4 # best estimate from images
        # filename is the output file name
        metadata['filename'] = '111012VirtTu' # this may change with new numbering
...
def virtuosoTu_cdf_to_nc(cdf_filename,
              writefile=True,
