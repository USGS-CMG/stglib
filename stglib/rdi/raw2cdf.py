...
def raw_to_cdf(metadata):
    ...
    ensemble_count, netcdf_index, ens_error, alldata = rdiadcpy.convert_pd0_to_netcdf(basefile, [0, -1], '0', 'CF', None)
    ...
    ds['bindist'].attrs['NOTE'] = "distance is calculated from center of bin 1 and bin size"
    ...
    for i in range(4):
        ...
        ds[varname].attrs['long_name'] = f"Beam {i+1} velocity (mm s-1)"
    ...
    ds['Hdg'].attrs['long_name'] = "INST Heading"
    ds['Hdg'].attrs['epic_code'] = 1215
    ds['Hdg'].attrs['heading_alignment'] = alldata['FLeader']['Heading_Alignment_Hundredths_of_Deg']
    ds['Hdg'].attrs['heading_bias'] = alldata['FLeader']['Heading_Bias_Hundredths_of_Deg']
    if alldata['FLeader']['Heading_Bias_Hundredths_of_Deg'] == 0:
        ds['Hdg'].attrs['NOTE_9'] = "no heading bias was applied by EB during deployment or by wavesmon"
    else:
        ds['Hdg'].attrs['NOTE_9'] = "a heading bias was applied by EB during deployment or by wavesmon"

    # varobj = cdf.createVariable('Ptch', 'f4', ('time',), fill_value=floatfill)
    ds['Ptch'].attrs['units'] = "hundredths of degrees"
    ds['Ptch'].attrs['long_name'] = "INST Pitch"
    ...
    ds['S'].attrs['long_name'] = "SALINITY (PPT)"
