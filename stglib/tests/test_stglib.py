...
class TestAqd(unittest.TestCase):
    def setUp(self):
        ...
        self.vel1 = np.expand_dims([0.23, 0.23, 0.23, 0.23], 1)
        self.vel2 = np.expand_dims([-0.52, -0.52, -0.52, -0.52], 1)
        self.vel3 = np.expand_dims([0.12, 0.12, 0.12, 0.12], 1)

        self.h = np.array([0, 10, 230, 230])
        self.p = np.array([0, -5, 5, 5])
        self.r = np.array([0, 3, -3, -3])
