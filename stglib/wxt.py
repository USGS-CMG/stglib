...
def cdf_to_nc(cdf_filename):
    ...
    if "WD_min" in ds:
        ...
        ds["WD_min"].values[ds["WD_min"].values < 0.] = ds["WD_min"].values[ds["WD_min"].values <0.]+360.
        ds["WD_min"].values[ds["WD_min"].values > 360.] = ds["WD_min"].values[ds["WD_min"].values > 360.]-360.
    
    if "WD_410" in ds:
        ds["WD_410"].values=ds["WD_410"].values + ds.attrs["magnetic_variation"]
        ds["WD_410"].values[ds["WD_410"].values < 0.] = ds["WD_410"].values[ds["WD_410"].values <0.]+360.
        ds["WD_410"].values[ds["WD_410"].values > 360.] = ds["WD_410"].values[ds["WD_410"].values > 360.]-360.
    
    if "WD_gust" in ds:
        ds["WD_gust"].values=ds["WD_gust"].values + ds.attrs["magnetic_variation"]
        ds["WD_gust"].values[ds["WD_gust"].values < 0.] = ds["WD_gust"].values[ds["WD_gust"].values <0.]+360.
        ds["WD_gust"].values[ds["WD_gust"].values > 360.] = ds["WD_gust"].values[ds["WD_gust"].values > 360.]-360.
...
def ds_add_attrs(ds):
    ...
    if "WD_min" in ds:
        ds["WD_min"].attrs.update({"units": "degrees", "long_name": "minimum wind from direction"})
    
    if "WD_410" in ds:
        ds["WD_410"].attrs.update({"units": "degrees","long_name": "mean wind from direction","standard_name": "wind_from_direction", "epic_code": "410"})
    
    if "WD_gust" in ds:
        ds["WD_gust"].attrs.update({"units": "degrees", "long_name": "maximum wind from direction", "standard_name": "wind_gust_from_direction"})
    
    if "WS_min" in ds:
        ds["WS_min"].attrs.update({"units": "m/s", "long_name": "minimum wind speed"})
    
    if "WS_401" in ds:
        ds["WS_401"].attrs.update({"units": "m/s", "long_name": "mean wind speed", "standard_name": "wind_speed", "epic_code": "401"})
    
    if "WG_402" in ds:
        ds["WG_402"].attrs.update({"units": "m/s", "long_name": "maximum wind speed", "standard_name": "wind_speed_of_gust", "epic_code":"402"})
    
    if "T_21" in ds:
        ds["T_21"].attrs.update({"units": "C", "standard_name": "air_temperature", "epic_code":"21"})
    
    if "RH_910" in ds:
        ds["RH_910"].attrs.update({"units": "%", "standard_name": "relative_humidity", "epic_code":"910"})
    
    if "BPR_915" in ds:
        ds["BPR_915"].attrs.update({"units": "pascals", "standard_name": "air_pressure", "epic_code":"915"}) 
    
    if "Rn_963" in ds:
        ds["Rn_963"].attrs.update({"units": "mm", "standard_name": "thickness_of_rainfall_amount", "epic_code":"963"})
        ...
        var.attrs.update(
            {
                ...
                "initial_instrument_height_note": dsattrs["initial_instrument_height_note"],
