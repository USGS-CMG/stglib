...
def rename_columns(df):
    ...
    df.rename(columns={"(__Y__,_ __(_)__)___(__M__,_ __(_)__)___(__D__,_ __(_)__)___(__H__,_ __(_)__)___(__M__,_ __(_)_._1__)___(__S__,_ __(_)__)":
            "time"},
            inplace=True)
...
def read_vel_snr_std(filbase):
    ...
    ds['vx'] = xr.DataArray(np.vstack(tuple(ds['Cell%02d_Vx' % (k+1)] for k in range(numbins))).T, dims=('time', 'bindist'))
    ds['vy'] = xr.DataArray(np.vstack(tuple(ds['Cell%02d_Vy' % (k+1)] for k in range(numbins))).T, dims=('time', 'bindist'))
    ds['spd'] = xr.DataArray(np.vstack(tuple(ds['Cell%02d_Spd' % (k+1)] for k in range(numbins))).T, dims=('time', 'bindist'))
    ds['dir'] = xr.DataArray(np.vstack(tuple(ds['Cell%02d_Dir' % (k+1)] for k in range(numbins))).T, dims=('time', 'bindist'))
    for v in ds.data_vars:
        if '_Vx' in v or '_Vy' in v or '_Spd' in v or '_Dir' in v:
            ds = ds.drop(v)

    snr = pd.read_csv(filbase + '.snr',
                      delim_whitespace=True,
                      header=[0,1],
                      parse_dates=[[1,2,3,4,5,6]],
                      date_parser=aqdutils.date_parser)
    ...
    for var in ['SNR1', 'SNR2']:
        ds[var.lower()] = xr.DataArray(np.vstack(tuple(snr['Cell%02d_%s' % (k+1, var)] for k in range(numbins))).T, dims=('time', 'bindist'))

    std = pd.read_csv(filbase + '.std',
                      delim_whitespace=True,
                      header=[0,1],
                      parse_dates=[[1,2,3,4,5,6]],
                      date_parser=aqdutils.date_parser)
    ...
    for var in ['Errx', 'Erry']:
        ds[var.lower()] = xr.DataArray(np.vstack(tuple(std['Cell%02d_%s' % (k+1, var)] for k in range(numbins))).T, dims=('time', 'bindist'))
