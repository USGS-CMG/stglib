...
- ``<VAR>_comment``: an additional comment about the variable that may be useful.
...
- ``<VAR>_mask_expr``: trim values based on an expression containing another variable. For example, ``Turb_mask_expr: "P_1ac < 0.1"`` will fill all ``Turb`` data where ``P_1ac`` is less than 0.1. A list of masks is also supported (``WG_402_mask_expr: ["WG_402 == 0", "WD_410 == 0"]``) and will be interpreted as "AND" between each mask. Currently supported operators are ['>', '<', '>=', '<=', '==', '!=']. Usage is currently limited to simple expressions with the masking variable on the left-hand side.
...
- ``AnalogInput1: "true"`` or ``AnalogInput2: "true"``: set these values if deployed with external analog sensors.
...
- ``average_tide_interval``: enter the desired tide interval when using the .wb file
- ``average_tide_interval_units``: tide interval units
- ``average_tide_duration``: enter the desired tide duration when using the .wb file
- ``average_tide_duration_units``: tide duration units

For Seagauge waves:

- ``average_wave_interval``: enter the desired wave interval
- ``average_wave_interval_units``: wave interval units
- ``wp_min``, ``wp_max``: min/max allowable wave period, in seconds
- ``wh_min``, ``wh_max``: min/max allowable wave height, in meters
- ``wp_ratio``: maximum allowable ratio between peak period (``wp_peak``) and mean period (``wp_4060``)
